...
export default function CompareSlot({ slot, accent, onUpdate, onClear, autoLoadName }) {
  ...
  async function runFullSearch(searchName) {
    ...
    try {
      ...
      onUpdate({
        loading: false, error: null, compoundName: searchName,
        features: feats, result: pred,
      })
    } catch (err) {
      onUpdate({
        loading: false, error: err.message || 'Lookup failed',
        compoundName: searchName, features: null, result: null,
      })
    }
  }
      ...
      <div className="flex-1 flex flex-col items-center justify-center min-h-[280px]">
        {loading ? (
          ...
          <div className="w-full flex flex-col items-center">
            ...
            {/* Compound name + verdict badge */}
            <div className="flex items-center gap-2 flex-wrap justify-center mb-2">
              <h3 className="text-base font-bold text-white capitalize text-center">
                {compoundName}
              </h3>
